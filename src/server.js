...
// Serve the pre-built static installer.
// Build once with Inno Setup and set INSTALLER_PATH in .env.
// In production, host the .exe on Azure Blob and set INSTALLER_BLOB_URL instead.
app.get("/download", (req, res) => {
  const installerPath = process.env.INSTALLER_PATH || path.join(__dirname, "dist", "TallyAgent-Setup.exe");
  if (!fs.existsSync(installerPath)) {
    return res.status(503).send("Installer not yet built. Run Inno Setup on templates/installer.iss first.");
  }
  res.download(installerPath, "TallyAgent-Setup.exe", (err) => {
